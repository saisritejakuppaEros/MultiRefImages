...
# Test run: short steps for quick validation. Set TEST_RUN=0 for full run.
TEST_RUN="${TEST_RUN:-1}"

if [ "$TEST_RUN" = "1" ]; then
  echo "=== TEST RUN (short steps) ==="
  # Phase 1: 50 steps, checkpoint every 25
  PHASE1_STEPS=50
  PHASE1_CKPT_STEPS=25
  PHASE1_DIR="log/phase1_test"
  # Phase 2: 100 steps, checkpoint every 50
  PHASE2_STEPS=100
  PHASE2_CKPT_STEPS=50
  PHASE2_DIR="log/phase2_test"
  PHASE2_PHASE1_CKPT="${PHASE1_DIR}/checkpoint-${PHASE1_STEPS}/hybrid_lora_layout.safetensors"
  # Phase 3: 150 steps, checkpoint every 75
  PHASE3_STEPS=150
  PHASE3_CKPT_STEPS=75
  PHASE3_DIR="log/phase3_test"
...
PHASE3_LAYOUT_WARMUP="${PHASE3_LAYOUT_WARMUP:-1000}"
...
echo ""
echo "For full run: TEST_RUN=0 ./run_phases.sh"
